...
void rotate(int degrees, int speed)
{
	...
	*/
	...
	rotTicks = abs((degrees * turnTicks)/90);//while the average of the two encoders is less than the parameter
	while(abs(nMotorEncoder[rightMotor]) < rotTicks || abs(nMotorEncoder[leftMotor]) < rotTicks){
		if (abs(nMotorEncoder[rightMotor]) > rotTicks){
			motor[rightMotor] = 0;
		}
		else if (abs(nMotorEncoder[leftMotor]) > rotTicks){
			motor[leftMotor] = 0;
		}
		else {
			if(abs(rotTicks) + rotTicks == 0){
				motor[rightMotor] = -speed;
				motor[leftMotor] = speed;
			}
			else {
...
------------------------------------------------------*/
		...
		else if ((vexRT[Btn8R] == 1) && (isPlayed == false)){ //if autonomous has not been played and Btn 8R is pressed
			forwardDistance(3); //go forward 2.25 tiles
